          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td>{money(row.cashAmount)}</td>
                <td><strong>{money(row.amount)}</strong></td>
                <td>{row.id ? <button type="button" className={styles.previewButton} onClick={() => preview(String(row.proofUrl ?? ""), {
                      scope:
                        String(row.kind ?? "").toUpperCase() === "BANK_DEPOSIT"
                          ? "deposit"
                          : String(row.kind ?? "").toUpperCase() === "EXPENSE"
                            ? "expense"
                            : "transaction",
                      id: String(row.id),
