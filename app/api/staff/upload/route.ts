...
function safeOriginalName(value: string): string {
  return value.replace(/[^\w.\- ()]+/g, "_").slice(0, 240) || "upload";
}
...
async function compressImage(input: Buffer, kind: string): Promise<Buffer> {
  try {
    const sharpModule = await import("sharp");
    const sharp = sharpModule.default;
    const profile = kind === "PROFILE";

    const first = await sharp(input, { failOn: "none" })
      .rotate()
      .resize({
        width: profile ? 720 : 1800,
        height: profile ? 720 : 1800,
        fit: "inside",
        withoutEnlargement: true,
      })
      .webp({
        quality: profile ? 78 : 82,
        effort: 4,
      })
      .toBuffer();

    if (profile && first.length > MAX_PROFILE_SIZE) {
    ...
    console.warn("STAFF_IMAGE_COMPRESSION_FALLBACK:", error);
    return input;
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const kind = cleanText(form.get("kind")).toUpperCase() || "OTHER";
    ...
    if (!ALLOWED_KINDS.has(kind)) {
      return NextResponse.json(
        { success: false, message: "The selected upload category is invalid." },
        { status: 400 },
      );
    }
    ...
    if (kind === "PROFILE" && !file.type.startsWith("image/")) {
      return NextResponse.json(
        {
          success: false,
          message: "A profile image must be JPG, PNG or WEBP.",
        },
        { status: 415 },
      );
    }

    if (file.size <= 0 || file.size > MAX_ORIGINAL_SIZE) {
      return NextResponse.json(
        {
          success: false,
    ...
    const compressed = shouldCompress
      ? await compressImage(original, kind)
      : original;
    const useCompressed = shouldCompress && compressed.length < original.length;
    const finalBuffer = useCompressed ? compressed : original;
    const finalMimeType = useCompressed ? "image/webp" : file.type;

    if (kind === "PROFILE" && finalBuffer.length > MAX_PROFILE_SIZE) {
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    const compressionRatio =
      originalSizeBytes > 0
        ? Number(((1 - sizeBytes / originalSizeBytes) * 100).toFixed(2))
        : 0;
    const checksumSha256 = createHash("sha256")
      .update(finalBuffer)
      .digest("hex");

    const record = await prisma.staffFile.create({
      data: {
    ...
    console.error("STAFF_UPLOAD_ERROR:", error);
