...
const navigation: Array<{
  ...
  { page: "Staff Expense Requests", glyph: "receipt", group: "Expenses" },
  ...
  { page: "Performance Reports", glyph: "profit", group: "Reports" },
  { page: "Lock Accounting Periods", glyph: "lock", group: "Reports" },
  { page: "Staff Funding Ledger", glyph: "float", group: "Operations" },
  { page: "Attendance Management", glyph: "attendance", group: "Operations" },
  { page: "Attendance Progress", glyph: "income", group: "Operations" },
  { page: "Fingerprint Devices", glyph: "user", group: "Operations" },
  { page: "SMS & Proof Review", glyph: "report", group: "Verification" },
  { page: "Admin Verification Documents", glyph: "balance", group: "Verification" },
...
function Glyph({
  ...
  return (
    <span
      className={`${styles.glyph} ${styles.materialSymbol} material-symbols-rounded ${className}`}
      aria-hidden="true"
    >
      {glyphs[name]}
    </span>
...
function iconForText(value: string) {
  const text = value.toLowerCase();
  const rules: Array<[string[], string]> = [
    [["success", "approved", "verified", "confirmed", "complete", "completed", "present", "balanced"], "check_circle"],
    [["pending", "waiting", "queued", "processing"], "schedule"],
    [["rejected", "failed", "error", "cancelled", "absent", "blocked"], "cancel"],
    [["warning", "mismatch", "missing", "hold"], "warning"],
    [["save", "submit", "post", "create", "add", "new"], "add_task"],
    [["update", "refresh", "reload", "sync"], "sync"],
    [["export", "download", "csv", "excel", "pdf"], "download"],
    [["print"], "print"],
    [["view", "preview", "open file", "document link"], "visibility"],
    [["delete", "remove", "reject"], "delete"],
    [["dashboard", "overview"], "dashboard"],
    [["income", "revenue", "cash in", "deposit"], "payments"],
    [["expense", "spent", "cash out", "cost"], "receipt_long"],
    [["balance", "account"], "account_balance_wallet"],
    [["date", "day", "period", "month", "year", "calendar"], "calendar_month"],
    [["staff", "user", "employee", "name", "person"], "person"],
    [["email", "mail"], "mail"],
    [["phone", "contact"], "call"],
    [["reference", "number", "code"], "tag"],
    [["status", "state"], "verified"],
    [["amount", "total", "value"], "calculate"],
    [["float", "funding"], "currency_exchange"],
    [["attendance", "present", "absent", "late"], "how_to_reg"],
    [["bank", "statement", "reconciliation"], "account_balance"],
    [["report", "document", "proof", "receipt", "file"], "description"],
    [["notification", "message", "sms"], "notifications_active"],
    [["approval", "approve", "review", "decision", "verify"], "fact_check"],
    [["ledger", "book", "journal", "transaction"], "menu_book"],
    [["profit", "performance", "score", "progress", "rate"], "monitoring"],
    [["branch", "company", "office"], "business"],
    [["role", "permission"], "admin_panel_settings"],
    [["password", "secret", "lock"], "lock"],
    [["profile", "image", "photo"], "account_circle"],
    [["search", "filter"], "filter_alt"],
    [["note", "reason", "description", "purpose"], "edit_note"],
    [["opening", "open"], "lock_open"],
    [["closing", "close"], "lock"],
  ];

  return rules.find(([keywords]) => keywords.some((keyword) => text.includes(keyword)))?.[1] ?? "auto_awesome";
}
...
function MaterialIcon({
  ...
  return (
    <span
      className={`${styles.materialSymbol} material-symbols-rounded ${styles.contextIcon} ${className}`}
      aria-hidden="true"
    >
      {iconForText(text)}
    </span>
...
function IconText({
  ...
  return (
    <span className={`${styles.iconText} ${className}`}>
...
function readableNodeText(node: ReactNode): string {
  if (typeof node === "string" || typeof node === "number") {
    return String(node);
  }
  if (Array.isArray(node)) {
    return node.map(readableNodeText).join(" ");
  }
...
function decorateTableHeaders(node: ReactNode): ReactNode {
  return Children.map(node, (child) => {
    if (!isValidElement<{ children?: ReactNode }>(child)) return child;

    const decoratedChildren = decorateTableHeaders(child.props.children);

    if (child.type === "th") {
      const label = readableNodeText(child.props.children).trim() || "Column";
      return cloneElement(
        child,
        undefined,
        <span className={styles.tableHeaderLabel}>
          <MaterialIcon text={label} />
          <span>{decoratedChildren}</span>
        </span>,
      );
    }

    return cloneElement(child, undefined, decoratedChildren);
  });
}
...
export default function AccountantDashboardClient({ user }: Props) {
  ...
  useEffect(() => {
    const stylesheetId = "simamia-material-symbols-rounded";
    ...
    if (document.fonts) {
      void document.fonts.load('20px "Material Symbols Rounded"');
    }
      ...
      >
        <div className={styles.brand}>
          ...
          <div>
            ...
            <small><MaterialIcon text="Accountant Portal" />Accountant Portal</small>
          </div>
        ...
        <button
          ...
          className={`${styles.collapseControl} ${
            collapsed ? styles.collapseControlCollapsed : ""
          }`}
          onClick={() => {
...
function OperationsPage({ module }: { module: OperationsCentre.OperationsModule }) {
  return <OperationsCentre.AccountantOperationsCentre initialModule={module} />;
}
...
function PageContent({
  ...
  if (page === "Manual Cashflow") {
    return <OperationsPage module="manual-cashflow" />;
  }
  ...
  if (page === "Bank Reconciliation") {
    return <OperationsPage module="bank-reconciliation" />;
  }

  if (page === "Expense Approval") {
    return <OperationsPage module="expense-approval" />;
  }
  ...
  if (page === "Financial Reports") {
    return <OperationsPage module="financial-reports" />;
  }
  ...
  if (page === "Attendance Management") {
    return <OperationsPage module="attendance-register" />;
  }

  if (page === "Staff Expense Requests") {
    return <OperationsPage module="expense-requests" />;
  }

  if (page === "Staff Funding Ledger") {
    return <OperationsPage module="funding-ledger" />;
  }

  if (page === "Attendance Progress") {
    return <OperationsPage module="attendance-progress" />;
  }

  if (page === "Fingerprint Devices") {
    return <OperationsPage module="fingerprint" />;
  }

  if (page === "SMS & Proof Review") {
    return <OperationsPage module="proof-review" />;
  }

  if (page === "Admin Verification Documents") {
    return <OperationsPage module="admin-documents" />;
  }

  if (page === "Performance Reports") {
    return <OperationsPage module="performance-reports" />;
  }
...
function DashboardHome({
  ...
  const staff = safeArray<any>(data.users).filter((item) =>
    ["STAFF", "BROKER", "GPS_MANAGER"].includes(
      safeText(item.role).toUpperCase(),
    ),
  );
  const transactions = safeArray<any>(data.recentTransactions).slice(0, 5);
  const floats = safeArray<any>(data.floats);
  const attendance = safeArray<any>(data.attendance);
  const settledFloats = floats.filter((item) =>
    ["CONFIRMED", "RETURNED", "DEPOSITED", "APPROVED"].includes(
      safeText(item.status).toUpperCase(),
    ),
  ).length;
  const presentSessions = attendance.reduce((total, item) => {
    const morning = ["PRESENT", "LATE"].includes(
      safeText(item.morningStatus || item.status).toUpperCase(),
    );
    const evening = ["PRESENT", "LATE"].includes(
      safeText(item.eveningStatus || item.status).toUpperCase(),
    );
    return total + Number(morning) + Number(evening);
  }, 0);
  const possibleSessions = attendance.length * 2;
  const floatProgress = floats.length
    ? Math.round((settledFloats / floats.length) * 100)
    : 0;
  const attendanceProgress = possibleSessions
    ? Math.round((presentSessions / possibleSessions) * 100)
    : 0;
  const dailyIncome = Number(data.stats.dailyIncome || 0);
  const dailyExpenses = Number(data.stats.dailyExpenses || 0);
  const dailyNet = Number(data.stats.dailyNetProfit || 0);
  const netRate = dailyIncome
    ? Math.round((dailyNet / Math.max(dailyIncome, 1)) * 1000) / 10
    : 0;

  return (
    <section className={styles.dashboardReference}>
      <header className={styles.dashboardWelcome}>
        <div>
          <small>SIMAMIA ACCOUNTANT WORKSPACE</small>
          <h2>
            Welcome, {safeText(data.accountant?.name) || "Accountant"} <span>🔥</span>
          </h2>
          <p>Track today&apos;s money, staff funding, approvals and accounting controls.</p>
        </div>
        <div className={styles.welcomeActions}>
          <span className={styles.dateChip}>
            <Glyph name="calendar" /> {formatDate(new Date())}
          </span>
          <button type="button" onClick={() => openPage("Financial Reports")}>
            <Glyph name="download" /> Export
          </button>
        </div>
      </header>

      <div className={styles.referenceDashboardGrid}>
        <div className={styles.referenceLeftColumn}>
          <article className={`${styles.referenceCard} ${styles.balanceWalletCard}`}>
            <header className={styles.referenceCardHeader}>
              <div>
                <span>Available Balance</span>
                <small>Live company cash position</small>
              </div>
              <span className={styles.compactSelect}>
                <Glyph name="calendar" /> Today
              </span>
            </header>
            <strong className={styles.walletAmount}>
              {money(data.stats.totalBalance)}
            </strong>
            <div className={styles.walletMeta}>
              <span>{data.currentDay ? `${data.currentDay.status} financial day` : "No financial day open"}</span>
              <b className={netRate >= 0 ? styles.positivePill : styles.negativePill}>
                {netRate >= 0 ? "↑" : "↓"} {Math.abs(netRate)}%
              </b>
            </div>
            <div className={styles.walletReference}>
              <span>Account control</span>
              <strong>{safeText(data.company?.code) || "SIMAMIA-FINANCE"}</strong>
              <button type="button" title="Open cash book" onClick={() => openPage("Cash Book")}>
                <Glyph name="eye" />
              </button>
            </div>
            <div className={styles.walletActions}>
              <button type="button" onClick={() => openPage("Manual Cashflow")}>
                <Glyph name="arrow" /> Issue Float
              </button>
              <button type="button" onClick={() => openPage("Cash Book")}>
                <Glyph name="cash" /> Receive Cash
              </button>
              <button type="button" aria-label="Open quick actions" onClick={() => openPage("Open Financial Day")}>
                <Glyph name="plus" />
              </button>
            </div>
          </article>
          ...
          <article className={styles.referenceCard}>
            <header className={styles.referenceCardHeader}>
              <div>
                <h3>Quick staff funding</h3>
                <small>Choose a staff officer to issue float or cash</small>
              </div>
              <button className={styles.textAction} type="button" onClick={() => openPage("Staff Funding Ledger")}>
                This week <Glyph name="arrow" />
              </button>
            </header>
            <div className={styles.quickPeople}>
              <button type="button" className={styles.addPerson} onClick={() => openPage("Manual Cashflow")}>
                <Glyph name="plus" />
              </button>
              {staff.slice(0, 6).map((person, index) => (
                <button
                  type="button"
                  key={person.id}
                  title={`Fund ${person.name || person.email}`}
                  className={styles.quickPerson}
                  onClick={() => openPage("Manual Cashflow")}
                >
                  <span>
                    {person.profileImageUrl ? (
                      <img src={person.profileImageUrl} alt={person.name || "Staff"} />
                    ) : (
                      safeText(person.name || person.email || "S").slice(0, 1).toUpperCase()
                    )}
                  </span>
                  {index === 1 && <em>{safeText(person.name).split(" ")[0] || "Staff"}</em>}
                </button>
              ))}
              {!staff.length && <small className={styles.noStaffHint}>No active staff found</small>}
            </div>
          </article>

          <article className={`${styles.referenceCard} ${styles.activityCard}`}>
            <header className={styles.referenceCardHeader}>
              <div>
                <h3>Recent activity</h3>
                <small>Latest posted accountant transactions</small>
              </div>
              <button className={styles.textAction} type="button" onClick={() => openPage("General Ledger")}>
                View all <Glyph name="arrow" />
              </button>
            </header>
            <div className={styles.referenceActivityHead}>
              <span>Name</span><span>Date</span><span>Amount</span>
            </div>
            <div className={styles.referenceActivityList}>
              {transactions.map((item) => {
                const incoming = Number(item.debit || 0) > 0;
                const personName = item.user?.name || item.user?.email || item.description || "System transaction";
                return (
                  <article key={item.id}>
                    <span className={styles.referenceActivityAvatar}>
                      {item.user?.profileImageUrl ? (
                        <img src={item.user.profileImageUrl} alt={personName} />
                      ) : (
                        safeText(personName).slice(0, 1).toUpperCase()
                      )}
                    </span>
                    <div>
                      <strong>{personName}</strong>
                      <small>{item.user?.email || item.description || "Accounting entry"}</small>
                    </div>
                    <time>{formatDate(item.date)}</time>
                    <b className={incoming ? styles.activityPositive : styles.activityNegative}>
                      {incoming ? "+" : "-"}{moneyShort(incoming ? item.debit : item.credit)}
                    </b>
                  </article>
                );
              })}
              {!transactions.length && <Empty text="No recent accounting activity." />}
            </div>
          </article>
        </div>

        <div className={styles.referenceRightColumn}>
          <div className={styles.referenceTopStats}>
            <DashboardReferenceStat
              label="Today Spent"
              value={money(dailyExpenses)}
              note={`${data.stats.pendingExpenses || 0} requests awaiting approval`}
              positive={false}
              rate={dailyIncome ? Math.round((dailyExpenses / dailyIncome) * 1000) / 10 : 0}
            />
            <DashboardReferenceStat
              label="Today Income"
              value={money(dailyIncome)}
              note="Verified and posted company income"
              positive={dailyNet >= 0}
              rate={Math.abs(netRate)}
            />
          </div>

          <article className={`${styles.referenceCard} ${styles.cashflowReferenceCard}`}>
            <header className={styles.referenceCardHeader}>
              <div>
                <h3>Cashflow</h3>
                <small>Total balance</small>
                <strong className={styles.cashflowBalance}>{money(data.stats.totalBalance)}</strong>
              </div>
              <div className={styles.cashflowHeaderTools}>
                <span><i className={styles.blueDot}></i> Income</span>
                <span><i className={styles.goldDot}></i> Expense</span>
                <button className={styles.textAction} type="button" onClick={() => openPage("Cash Flow Statement")}>
                  This month <Glyph name="arrow" />
                </button>
              </div>
            </header>
            <ReferenceCashflowChart rows={data.monthlySeries} />
          </article>

          <article className={`${styles.referenceCard} ${styles.goalsCard}`}>
            <header className={styles.referenceCardHeader}>
              <div>
                <h3>Operational progress</h3>
                <small>Live control completion for this accounting period</small>
              </div>
              <button className={styles.textAction} type="button" onClick={() => openPage("Performance Reports")}>
                This week <Glyph name="arrow" />
              </button>
            </header>
            <div className={styles.goalGrid}>
              <DashboardGoal
                title="Float settlement"
                target={`${settledFloats} of ${floats.length} records settled`}
                value={floatProgress}
                amount={money(data.stats.outstandingFloat)}
                button={() => openPage("Float Verification")}
              />
              <DashboardGoal
                title="Attendance coverage"
                target={`${presentSessions} of ${possibleSessions} verified sessions`}
                value={attendanceProgress}
                amount={`${attendanceProgress}%`}
                button={() => openPage("Attendance Progress")}
                alternate
              />
            </div>
          </article>
...
function DashboardReferenceStat({
  label,
  value,
  note,
  positive,
  ...
  return (
    <article className={`${styles.referenceCard} ${styles.referenceStatCard}`}>
      <header>
        <span className={styles.iconLabel}><MaterialIcon text={label} />{label}</span>
        <span className={styles.compactSelect}><Glyph name="calendar" /> Today</span>
      </header>
      <strong>{value}</strong>
      <footer>
        <b className={positive ? styles.positivePill : styles.negativePill}>
          {positive ? "↑" : "↓"} {Number.isFinite(rate) ? rate : 0}%
        </b>
        <small>{note}</small>
      </footer>
    </article>
...
function ReferenceCashflowChart({ rows }: { rows: any[] }) {
  const list = safeArray<any>(rows).slice(-9);
  if (!list.length) return <Empty text="No cashflow data is available." />;
  const width = 860;
  const height = 280;
  const left = 34;
  const right = 18;
  ...
  const max = Math.max(
    1,
    ...list.flatMap((item) => [Number(item.income || 0), Number(item.expense || 0)]),
  );
  const x = (index: number) => left + (index * (width - left - right)) / Math.max(1, list.length - 1);
  const y = (value: number) => height - bottom - (value / max) * (height - top - bottom);
  const line = (key: "income" | "expense") => list.map((item, index) => `${x(index)},${y(Number(item[key] || 0))}`).join(" ");
  const incomeArea = `${left},${height - bottom} ${line("income")} ${width - right},${height - bottom}`;

  return (
    <div className={styles.referenceChart}>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Income and expense cashflow chart">
        <defs>
          <linearGradient id="referenceIncomeArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#0867f2" stopOpacity="0.18" />
            <stop offset="100%" stopColor="#0867f2" stopOpacity="0" />
          </linearGradient>
        </defs>
        {[0, 1, 2, 3, 4].map((index) => {
          const lineY = top + (index * (height - top - bottom)) / 4;
          return <line key={index} x1={left} x2={width - right} y1={lineY} y2={lineY} className={styles.referenceGridLine} />;
        })}
        <polygon points={incomeArea} className={styles.referenceIncomeArea} />
        <polyline points={line("income")} className={styles.referenceIncomeLine} />
        <polyline points={line("expense")} className={styles.referenceExpenseLine} />
        {list.map((item, index) => (
          <g key={item.key || index}>
            <circle cx={x(index)} cy={y(Number(item.income || 0))} r="4.5" className={styles.referenceIncomePoint} />
            <circle cx={x(index)} cy={y(Number(item.expense || 0))} r="4" className={styles.referenceExpensePoint} />
          </g>
        ))}
      </svg>
      <div className={styles.referenceChartLabels}>
        {list.map((item, index) => <span key={item.key || index}>{item.label || `P${index + 1}`}</span>)}
      </div>
    </div>
  );
}
...
function DashboardGoal({
  ...
  return (
    <button type="button" className={styles.goalItem} onClick={button}>
      <span className={alternate ? styles.goalAccentGold : styles.goalAccentBlue}></span>
      ...
      <div className={styles.goalProgress}>
        <span className={alternate ? styles.goalFillGold : styles.goalFillBlue} style={{ width: `${safeValue}%` }}></span>
        <em>{safeValue}%</em>
      </div>
    </button>
  );
...
function MetricCard({
  ...
  return (
    <article className={`${styles.metricCard} ${styles[theme]}`}>
      <span className={styles.metricCardIcon}><MaterialIcon text={label} /></span>
      <div className={styles.metricCardCopy}>
        <span>{label}</span>
        <strong>{value}</strong>
        <small>{note}</small>
      </div>
...
function GlassCard({
  ...
  return (
    <article className={styles.glassCard}>
      <header>
        <span className={styles.cardHeaderIcon}><MaterialIcon text={title} /></span>
        <div>
          <h3>{title}</h3>
  ...
  return (
    <div className={styles.lineChart}>
      <div className={styles.chartLegend}>
        <span>
          <MaterialIcon text="Income" /><i className={styles.legendIncome}></i>Income
        </span>
        <span>
          <MaterialIcon text="Expenses" /><i className={styles.legendExpense}></i>Expenses
        </span>
        <span>
          <MaterialIcon text="Deposits" /><i className={styles.legendDeposit}></i>Deposits
        </span>
      </div>
...
function CashFlowChart({ rows }: { rows: any[] }) {
  ...
  return (
    <div className={styles.cashFlowChart}>
      <div className={styles.chartLegend}>
        <span>
          <MaterialIcon text="Cash in" /><i className={styles.legendIncome}></i>Cash in
        </span>
        <span>
          <MaterialIcon text="Cash out" /><i className={styles.legendExpense}></i>Cash out
        </span>
      </div>
      <svg
        viewBox={`0 0 ${width} ${height}`}
...
function DataTable({
  ...
  return (
    <div className={styles.tableScroll}>
      <table style={{ minWidth }}>{decorateTableHeaders(children)}</table>
    </div>
  );
}
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className={styles.field}>
      <span className={styles.fieldLabel}><MaterialIcon text={label} />{label}</span>
      {children}
    </label>
  );
}
...
function CardHeading({ title, text }: { title: string; text: string }) {
  return (
    <div className={styles.cardHeading}>
      <span className={styles.cardHeadingIcon}><MaterialIcon text={title} /></span>
...
function Status({ status }: { status: unknown }) {
  ...
  return (
    ...
    >
      <MaterialIcon text={value || "Status"} className={styles.statusIcon} />
      <span>{value.replaceAll("_", " ") || "N/A"}</span>
    </span>
  );
}
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className={styles.detail}>
      <span><MaterialIcon text={label} />{label}</span>
    ...
    <div className={styles.notificationPopup}>
      ...
      <footer>
        <button type="button" onClick={openAll}>
          <Glyph name="bell" /> Open notification center
        </button>
      </footer>
    </div>
  );
...
function LoadingState() {
  return (
    <div className={styles.stateCard}>
      <span className={styles.loadingIcon}><MaterialIcon text="Loading finance records" /></span>
      <span className={styles.loader}></span>
      <h2><IconText text="Loading finance records" /></h2>
      <p>
...
function ErrorState({
  ...
  return (
    <div className={styles.stateCard}>
      <Glyph name="warning" />
      <h2><IconText text="Accountant portal could not load" /></h2>
      <p>{message}</p>
      <button type="button" onClick={retry}>
        <Glyph name="refresh" /> Try again
      </button>
    </div>
  );
}
...
function Empty({ text }: { text: string }) {
  return (
    <div className={styles.empty}>
      <MaterialIcon text={text} className={styles.emptyIcon} />
      <span>{text}</span>
    </div>
  );
}
...
namespace OperationsCentre {
  ...
  type Props = {
    initialModule: OperationsModule;
  };

  const moduleMeta: Record<OperationsModule, { title: string; subtitle: string; icon: string }> = {
    "expense-requests": {
      title: "Staff Expense Requests",
      subtitle: "Review every expense request submitted by an active STAFF user.",
      icon: "receipt_long",
    },
    "expense-approval": {
    ...
    "manual-cashflow": {
      title: "Float and Cash Assignment",
      subtitle: "Issue float, cash, or both to a STAFF user and calculate the combined amount automatically.",
      icon: "add_card",
    },
    "funding-ledger": {
      title: "Staff Funding Ledger",
      subtitle: "Combined float and cash totals for every STAFF user and the whole company.",
      icon: "currency_exchange",
    },
    "attendance-register": {
      title: "Attendance Journal",
      subtitle: "Only the Accountant records and verifies morning and evening STAFF attendance.",
      icon: "approval",
    },
    "attendance-progress": {
      title: "Attendance Progress",
      subtitle: "Compare attendance rates and identify the most present and most absent STAFF users.",
      icon: "monitoring",
    },
    fingerprint: {
      title: "Fingerprint Devices",
      subtitle: "Register devices, enrol STAFF users, rotate secrets, and control device access.",
      icon: "fingerprint",
    },
    "proof-review": {
      title: "SMS and Proof Review",
      subtitle: "Verify or reject STAFF-uploaded SMS messages and supporting documents with a reason.",
      icon: "fact_check",
    },
    "admin-documents": {
      title: "Company Admin Documents",
      subtitle: "Review comparison files and messages sent by the Company Admin.",
      icon: "difference",
    },
    "bank-reconciliation": {
      title: "Advanced Bank Reconciliation",
      subtitle: "Compare STAFF bank deposits with Company Admin verification documents.",
      icon: "account_balance",
    },
    "financial-reports": {
      title: "Financial Report Centre",
      subtitle: "Preview, print, and export combined financial data by date period.",
      icon: "lab_profile",
    },
    "performance-reports": {
      title: "Performance Report Centre",
      subtitle: "Review income, attendance, deposits, funding, transactions, and performance per STAFF user.",
      icon: "analytics",
    },
  };

  function tanzaniaToday() {
  ...
  async function requestJson<T = any>(url: string, options?: RequestInit): Promise<T> {
    ...
    if (!response.ok || result.success === false) {
      throw new Error(result.message || result.details || `Request failed (${response.status}).`);
    }
    return result as T;
  }

  function Status({ value }: { value: unknown }) {
    const status = text(value).toUpperCase() || "PENDING";
    return <span className={`${styles.opStatus} ${styles[`opStatus${status}`] || ""}`}><MaterialIcon text={status} /><span>{label(status)}</span></span>;
  }

  function Person({ row }: { row: any }) {
    const name = row?.name || row?.username || "User";
    return (
      <div className={styles.opPerson}>
        <span>
          {row?.profileImageUrl ? <img src={row.profileImageUrl} alt="" /> : String(name).slice(0, 1).toUpperCase()}
        </span>
        <div><strong>{name}</strong><small>{row?.email || row?.assignedRegion || "STAFF"}</small></div>
      </div>
    );
  }

  function Empty({ text: value }: { text: string }) {
    return <div className={styles.opEmpty}><MaterialIcon text={value} /><span>{value}</span></div>;
  }

  function Metric({ label: title, value, note }: { label: string; value: string; note?: string }) {
    return <article className={styles.opMetric}><span className={styles.opMetricIcon}><MaterialIcon text={title} /></span><div><span>{title}</span><strong>{value}</strong><small>{note || "Live database value"}</small></div></article>;
  }

  function ModuleHeader({ module }: { module: OperationsModule }) {
    const meta = moduleMeta[module];
    return (
      <header className={styles.opModuleHeader}>
        <span className={`${styles.materialSymbol} material-symbols-rounded`} aria-hidden="true">{meta.icon}</span>
        <div><small>SIMAMIA ACCOUNTANT CONTROL</small><h2>{meta.title}</h2><p>{meta.subtitle}</p></div>
      </header>
    );
  }

  export function AccountantOperationsCentre({ initialModule }: Props) {
    ...
    const load = useCallback(async (showLoader = true) => {
      if (showLoader) setLoading(true);
      try {
        setData(await requestJson(`/api/accountant/control-centre?${query}`));
      } catch (error) {
        setMessage(error instanceof Error ? error.message : "The enhanced accountant data could not load.");
      } finally {
        setLoading(false);
      }
    }, [query]);
    ...
    useEffect(() => {
      if (!message) return;
      const timer = window.setTimeout(() => setMessage(""), 4800);
    ...
    async function action(payload: Record<string, unknown>) {
      setBusy(true);
      try {
        const result = await requestJson<{ message: string }>("/api/accountant/control-centre", {
          method: "POST",
    ...
    async function upload(file: File) {
      ...
      const result = await requestJson<{ url: string }>("/api/accountant/upload", { method: "POST", body: form });
      return result.url;
    }
    ...
    return (
      <section className={styles.opWorkspace}>
        ...
        {loading ? <div className={styles.opLoading}>Loading verified company data…</div> : !data ? <Empty text="No data was returned." /> : (
          <div className={styles.opReveal}>
            {initialModule === "expense-requests" && <ExpenseRequests data={data} />}
  ...
  function FilterBar({ filter, setFilter, apply }: { filter: Filter; setFilter: (value: Filter) => void; apply: () => void }) {
    return (
      <section className={styles.opFilters}>
        <label><span><MaterialIcon text="Period" />Period</span><select value={filter.period} onChange={(e) => setFilter({ ...filter, period: e.target.value as Filter["period"] })}>
          <option value="DAY">Day</option><option value="WEEK">Week</option><option value="MONTH">Month</option><option value="YEAR">Year</option><option value="CUSTOM">Custom range</option>
        </select></label>
        {filter.period === "CUSTOM" ? <>
          <label><span><MaterialIcon text="From date" />From</span><input type="date" value={filter.from} max={filter.to} onChange={(e) => setFilter({ ...filter, from: e.target.value })} /></label>
          <label><span><MaterialIcon text="To date" />To</span><input type="date" value={filter.to} min={filter.from} onChange={(e) => setFilter({ ...filter, to: e.target.value })} /></label>
        </> : <label><span><MaterialIcon text="Calendar date" />Calendar date</span><input type="date" value={filter.anchor} onChange={(e) => setFilter({ ...filter, anchor: e.target.value })} /></label>}
        <label className={styles.opSearch}><span><MaterialIcon text="Search" />Search</span><input value={filter.q} onChange={(e) => setFilter({ ...filter, q: e.target.value })} onKeyDown={(e) => e.key === "Enter" && apply()} placeholder="Name, reference, category, SMS…" /></label>
        <button type="button" onClick={apply}>Apply filter</button>
      </section>
    );
  }

  function ExpenseRequests({ data }: { data: any }) {
    const rows = safeArray(data.expenses);
    return <div className={styles.opStack}>
      <div className={styles.opMetrics}>
        <Metric label="All requests" value={String(rows.length)} note={data.range?.label} />
        <Metric label="Pending" value={String(rows.filter((r: any) => r.status === "PENDING").length)} />
        <Metric label="Approved" value={String(rows.filter((r: any) => r.status === "APPROVED").length)} />
        <Metric label="Rejected" value={String(rows.filter((r: any) => r.status === "REJECTED").length)} />
      </div>
      <Table headers={["Date", "STAFF user", "Expense", "Purpose", "Amount", "Accountant", "Company Admin", "Final status", "Evidence"]}>
        {rows.map((row: any) => {
          const accountant = safeArray(row.decisions).find((item: any) => item.reviewerRole === "ACCOUNTANT");
          const admin = safeArray(row.decisions).find((item: any) => item.reviewerRole === "COMPANY_ADMIN");
          return <tr key={row.id}>
            <td>{date(row.expenseDate || row.createdAt, true)}</td><td><Person row={row.employee} /></td>
            <td><strong>{row.category}</strong><small>{label(row.requestMode)}</small></td>
  ...
  function ExpenseApproval({ data, busy, action }: { data: any; busy: boolean; action: (payload: Record<string, unknown>) => Promise<boolean> }) {
    const rows = safeArray(data.expenses);
    ...
    async function decide(decision: "APPROVED" | "REJECTED") {
      if (!selected) return;
      const ok = await action({ action: "REVIEW_EXPENSE", expenseId: selected.id, decision, reason });
      if (ok) setReason("");
    }
    return <div className={styles.opTwoColumn}>
      <article className={styles.opCard}><CardTitle title="Expense queue" text={`${rows.length} STAFF requests`} />
        <div className={styles.opQueue}>{rows.map((row: any) => <button type="button" key={row.id} className={selected?.id === row.id ? styles.opSelected : ""} onClick={() => setSelectedId(row.id)}>
      ...
      <article className={styles.opCard}><CardTitle title="Accountant decision" text="Any rejection rejects the expense; both approvals are required." />
        ...
        </div> : <Empty text="Select an expense request." />}
      </article>
    </div>;
  }

  function FundingIssue({ data, busy, action }: { data: any; busy: boolean; action: (payload: Record<string, unknown>) => Promise<boolean> }) {
    const staff = safeArray(data.staff);
    const [form, setForm] = useState({ staffId: staff[0]?.id || "", floatAmount: "", cashAmount: "", purpose: "Morning operational funding", referenceNo: "", note: "" });
    useEffect(() => { if (!form.staffId && staff[0]) setForm((current) => ({ ...current, staffId: staff[0].id })); }, [staff, form.staffId]);
    const combined = Number(form.floatAmount || 0) + Number(form.cashAmount || 0);
    async function submit(event: FormEvent) {
      event.preventDefault();
      const ok = await action({ action: "ISSUE_STAFF_FUNDS", ...form });
      if (ok) setForm((current) => ({ ...current, floatAmount: "", cashAmount: "", referenceNo: "", note: "" }));
    }
    return <div className={styles.opTwoColumn}>
      <form className={styles.opCard} onSubmit={submit}><CardTitle title="Issue float and cash" text="Both amounts are saved against the selected STAFF user." />
        <label className={styles.opField}><span>STAFF user</span><select required value={form.staffId} onChange={(e) => setForm({ ...form, staffId: e.target.value })}><option value="">Select STAFF user</option>{staff.map((row: any) => <option key={row.id} value={row.id}>{row.name} — {row.email}</option>)}</select></label>
        <div className={styles.opFormGrid}><label className={styles.opField}><span>Float amount</span><input type="number" min="0" value={form.floatAmount} onChange={(e) => setForm({ ...form, floatAmount: e.target.value })} /></label><label className={styles.opField}><span>Cash amount</span><input type="number" min="0" value={form.cashAmount} onChange={(e) => setForm({ ...form, cashAmount: e.target.value })} /></label></div>
        <div className={styles.opCombined}><span>Combined allocation</span><strong>{money(combined)}</strong></div>
        <label className={styles.opField}><span>Purpose</span><input required value={form.purpose} onChange={(e) => setForm({ ...form, purpose: e.target.value })} /></label>
        <label className={styles.opField}><span>Reference number</span><input value={form.referenceNo} onChange={(e) => setForm({ ...form, referenceNo: e.target.value })} placeholder="Generated automatically when empty" /></label>
        <label className={styles.opField}><span>Notes</span><textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} /></label>
        <button className={styles.opPrimaryButton} disabled={busy || !form.staffId || combined <= 0}>{busy ? "Saving…" : "Issue float and cash"}</button>
      </form>
      <article className={styles.opCard}><CardTitle title="Recent funding" text="Latest float and cash allocations" /><div className={styles.opList}>{safeArray(data.funding).slice(0, 25).map((row: any) => <article key={row.id}><Person row={row.staff} /><div><small>Float</small><strong>{money(row.floatAmount)}</strong></div><div><small>Cash</small><strong>{money(row.cashAmount)}</strong></div><b>{money(row.totalAmount)}</b><Status value={row.status} /></article>)}{!safeArray(data.funding).length && <Empty text="No combined funding has been issued." />}</div></article>
    </div>;
  }

  function FundingLedger({ data }: { data: any }) {
    const rows = safeArray(data.fundingTotals);
    const summary = data.reportSummary || {};
    return <div className={styles.opStack}><div className={styles.opMetrics}><Metric label="All STAFF users" value={String(summary.users || rows.length)} /><Metric label="Total float" value={money(summary.totalFloat)} /><Metric label="Total cash" value={money(summary.totalCash)} /><Metric label="Combined funding" value={money(Number(summary.totalFloat || 0) + Number(summary.totalCash || 0))} /></div>
  ...
  function AttendanceJournal({ data, busy, action }: { data: any; busy: boolean; action: (payload: Record<string, unknown>) => Promise<boolean> }) {
    ...
    useEffect(() => {
      ...
      for (const user of staff) {
        const record = safeArray(data.attendance).find((row: any) => String(row.userId) === String(user.id) && inputDate(row.date) === selectedDate);
        next[user.id] = { userId: user.id, morningStatus: record?.morningStatus || null, eveningStatus: record?.eveningStatus || null, morningTime: record?.checkInAt ? new Date(record.checkInAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "Africa/Dar_es_Salaam" }) : "08:00", eveningTime: record?.checkOutAt ? new Date(record.checkOutAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "Africa/Dar_es_Salaam" }) : "17:00", note: record?.notes || "" };
      }
      setDrafts(next);
    }, [data.attendance, staff, selectedDate]);
    function mark(userId: string, session: "morningStatus" | "eveningStatus", value: string) { setDrafts((current) => ({ ...current, [userId]: { ...current[userId], userId, [session]: value } })); }
    function bulk(value: string) { setDrafts((current) => Object.fromEntries(staff.map((user: any) => [user.id, { ...current[user.id], userId: user.id, morningStatus: value, eveningStatus: value }]))); }
    return <div className={styles.opStack}>
  ...
  function Session({ title, value, time, onMark, onTime }: { title: string; value: string | null; time: string; onMark: (value: string) => void; onTime: (value: string) => void }) {
    return <div className={styles.opSession}><strong>{title}</strong><div><button className={value === "PRESENT" ? styles.opPresent : ""} onClick={() => onMark("PRESENT")}>✓</button><button className={value === "LATE" ? styles.opLate : ""} onClick={() => onMark("LATE")}>L</button><button className={value === "ABSENT" ? styles.opAbsent : ""} onClick={() => onMark("ABSENT")}>×</button></div><input type="time" value={time} disabled={value === "ABSENT"} onChange={(e) => onTime(e.target.value)} /></div>;
  }

  function AttendanceProgress({ data }: { data: any }) {
    const rows = safeArray(data.attendanceProgress);
    const summary = data.attendanceSummary || {};
    return <div className={styles.opStack}><div className={styles.opMetrics}><Metric label="Attendance records" value={String(summary.records || 0)} /><Metric label="Morning present" value={String(summary.morningPresent || 0)} /><Metric label="Evening present" value={String(summary.eveningPresent || 0)} /><Metric label="Absent sessions" value={String(Number(summary.morningAbsent || 0) + Number(summary.eveningAbsent || 0))} /></div>
      <div className={styles.opLeaderGrid}><article className={styles.opCard}><CardTitle title="Most present STAFF" text="Highest recorded present sessions" />{summary.mostPresent ? <Leader row={summary.mostPresent} positive /> : <Empty text="No attendance data." />}</article><article className={styles.opCard}><CardTitle title="Most absent STAFF" text="Highest recorded absent sessions" />{summary.mostAbsent ? <Leader row={summary.mostAbsent} /> : <Empty text="No attendance data." />}</article></div>
      <Table headers={["STAFF user", "Present sessions", "Absent sessions", "Marked sessions", "Attendance rate", "Progress"]}>{rows.map((row: any) => <tr key={row.user?.id}><td><Person row={row.user} /></td><td>{row.present}</td><td>{row.absent}</td><td>{row.marked}</td><td><strong>{row.rate}%</strong></td><td><div className={styles.opProgress}><span style={{ width: `${Math.min(100, Math.max(0, row.rate))}%` }} /></div></td></tr>)}{!rows.length && <tr><td colSpan={6}><Empty text="No attendance progress data." /></td></tr>}</Table></div>;
  }

  function Leader({ row, positive = false }: { row: any; positive?: boolean }) {
    return <div className={`${styles.opLeader} ${positive ? styles.opPositive : ""}`}><Person row={row.user} /><strong>{positive ? row.present : row.absent} sessions</strong><span>{row.rate}% attendance rate</span></div>;
  }

  function FingerprintDevices({ notify }: { notify: (message: string) => void }) {
    const [data, setData] = useState<any>({ devices: [], enrolments: [], users: [] });
    const [busy, setBusy] = useState(false);
    const [deviceForm, setDeviceForm] = useState({ name: "", serialNumber: "", location: "", vendor: "" });
    const [enrolForm, setEnrolForm] = useState({ deviceId: "", userId: "", externalUserCode: "", fingerLabel: "Right thumb" });
    const [secret, setSecret] = useState("");
    const load = useCallback(async () => { try { setData(await requestJson("/api/accountant/fingerprint-devices")); } catch (error) { notify(error instanceof Error ? error.message : "Fingerprint data could not load."); } }, [notify]);
    useEffect(() => { void load(); }, [load]);
    async function post(payload: Record<string, unknown>) { setBusy(true); try { const result = await requestJson<any>("/api/accountant/fingerprint-devices", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); notify(result.message); if (result.deviceSecret) setSecret(result.deviceSecret); await load(); } catch (error) { notify(error instanceof Error ? error.message : "Fingerprint action failed."); } finally { setBusy(false); } }
    async function register(event: FormEvent) { event.preventDefault(); await post({ action: "REGISTER_DEVICE", ...deviceForm }); setDeviceForm({ name: "", serialNumber: "", location: "", vendor: "" }); }
    async function enrol(event: FormEvent) { event.preventDefault(); await post({ action: "ENROL_USER", ...enrolForm }); setEnrolForm((current) => ({ ...current, externalUserCode: "" })); }
    return <div className={styles.opStack}>{secret && <div className={styles.opSecret}><strong>Copy the device secret now</strong><code>{secret}</code><button onClick={() => void navigator.clipboard.writeText(secret)}>Copy</button></div>}
      <div className={styles.opTwoColumn}><form className={styles.opCard} onSubmit={register}><CardTitle title="Register fingerprint device" text="Only Accountant and Company Admin accounts can register devices." /><div className={styles.opFormGrid}><label className={styles.opField}><span>Device name</span><input required value={deviceForm.name} onChange={(e) => setDeviceForm({ ...deviceForm, name: e.target.value })} /></label><label className={styles.opField}><span>Serial number</span><input required value={deviceForm.serialNumber} onChange={(e) => setDeviceForm({ ...deviceForm, serialNumber: e.target.value })} /></label><label className={styles.opField}><span>Location</span><input value={deviceForm.location} onChange={(e) => setDeviceForm({ ...deviceForm, location: e.target.value })} /></label><label className={styles.opField}><span>Vendor</span><input value={deviceForm.vendor} onChange={(e) => setDeviceForm({ ...deviceForm, vendor: e.target.value })} /></label></div><button className={styles.opPrimaryButton} disabled={busy}>Register device</button></form>
      <form className={styles.opCard} onSubmit={enrol}><CardTitle title="Enrol STAFF fingerprint" text="Only active STAFF users are available for enrolment." /><label className={styles.opField}><span>Device</span><select required value={enrolForm.deviceId} onChange={(e) => setEnrolForm({ ...enrolForm, deviceId: e.target.value })}><option value="">Select device</option>{safeArray(data.devices).filter((r: any) => r.status === "ACTIVE").map((row: any) => <option key={row.id} value={row.id}>{row.name} — {row.serialNumber}</option>)}</select></label><label className={styles.opField}><span>STAFF user</span><select required value={enrolForm.userId} onChange={(e) => setEnrolForm({ ...enrolForm, userId: e.target.value })}><option value="">Select STAFF user</option>{safeArray(data.users).map((row: any) => <option key={row.id} value={row.id}>{row.name} — {row.email}</option>)}</select></label><div className={styles.opFormGrid}><label className={styles.opField}><span>Device user code</span><input required value={enrolForm.externalUserCode} onChange={(e) => setEnrolForm({ ...enrolForm, externalUserCode: e.target.value })} /></label><label className={styles.opField}><span>Finger label</span><input value={enrolForm.fingerLabel} onChange={(e) => setEnrolForm({ ...enrolForm, fingerLabel: e.target.value })} /></label></div><button className={styles.opPrimaryButton} disabled={busy}>Enrol fingerprint</button></form></div>
      <Table headers={["Device", "Serial", "Location", "Vendor", "Status", "Last seen", "Control"]}>{safeArray(data.devices).map((row: any) => <tr key={row.id}><td><strong>{row.name}</strong></td><td>{row.serialNumber}</td><td>{row.location || "—"}</td><td>{row.vendor || "—"}</td><td><Status value={row.status} /></td><td>{date(row.lastSeenAt, true)}</td><td><div className={styles.opTableActions}><button disabled={busy} onClick={() => void post({ action: "ROTATE_SECRET", deviceId: row.id })}>Rotate secret</button><button disabled={busy} onClick={() => void post({ action: "SET_DEVICE_STATUS", deviceId: row.id, status: row.status === "ACTIVE" ? "INACTIVE" : "ACTIVE" })}>{row.status === "ACTIVE" ? "Disable" : "Activate"}</button></div></td></tr>)}{!safeArray(data.devices).length && <tr><td colSpan={7}><Empty text="No fingerprint devices registered." /></td></tr>}</Table>
      <Table headers={["STAFF user", "Device", "External code", "Finger", "Active"]}>{safeArray(data.enrolments).map((row: any) => <tr key={row.id}><td><Person row={row.user} /></td><td>{safeArray(data.devices).find((d: any) => d.id === row.deviceId)?.name || row.deviceId}</td><td>{row.externalUserCode}</td><td>{row.fingerLabel || "—"}</td><td>{row.isActive ? "Yes" : "No"}</td></tr>)}{!safeArray(data.enrolments).length && <tr><td colSpan={5}><Empty text="No STAFF fingerprint enrolments." /></td></tr>}</Table>
    </div>;
  }
  ...
  function BankReview({ data, busy, action, upload, notify }: { data: any; busy: boolean; action: (payload: Record<string, unknown>) => Promise<boolean>; upload: (file: File) => Promise<string>; notify: (message: string) => void }) {
    const rows = safeArray(data.deposits); const [selectedId, setSelectedId] = useState(rows[0]?.id || ""); const selected = rows.find((row: any) => row.id === selectedId) || rows[0];
    const [form, setForm] = useState({ statementAmount: "", statementReference: "", statementDate: "", statementBankAccount: "", bankStatementUrl: "", packetId: "", reason: "" });
    useEffect(() => { if (!selected) return; const packet = safeArray(selected.packets)[0]; setForm({ statementAmount: selected.statementAmount == null ? String(selected.amount || "") : String(selected.statementAmount), statementReference: selected.statementReference || selected.referenceNo || "", statementDate: inputDate(selected.statementDate || selected.depositDate), statementBankAccount: selected.statementBankAccount || selected.bankAccount || "", bankStatementUrl: selected.bankStatementUrl || "", packetId: packet?.id || "", reason: selected.mismatchReason || "" }); }, [selected?.id]);
    async function file(event: ChangeEvent<HTMLInputElement>) { const selectedFile = event.target.files?.[0]; if (!selectedFile) return; try { const url = await upload(selectedFile); setForm((current) => ({ ...current, bankStatementUrl: url })); } catch (error) { notify(error instanceof Error ? error.message : "Statement upload failed."); } }
    async function submit(event: FormEvent) { event.preventDefault(); if (!selected) return; await action({ action: "REVIEW_BANK_DEPOSIT", depositId: selected.id, ...form }); }
    return <div className={styles.opTwoColumn}><article className={styles.opCard}><CardTitle title="Deposit queue" text={`${rows.length} STAFF deposit records`} /><div className={styles.opQueue}>{rows.map((row: any) => <button key={row.id} onClick={() => setSelectedId(row.id)} className={selected?.id === row.id ? styles.opSelected : ""}><Person row={row.staff} /><b>{money(row.amount)}</b><Status value={row.status} /></button>)}{!rows.length && <Empty text="No bank deposits." />}</div></article><form className={styles.opCard} onSubmit={submit}><CardTitle title="Compare bank records" text="Company Admin comparison document is required for verification." />{selected ? <><div className={styles.opCompare}><span>STAFF amount<strong>{money(selected.amount)}</strong></span><span>Reference<strong>{selected.referenceNo || "—"}</strong></span><span>Date<strong>{date(selected.depositDate)}</strong></span><span>Account<strong>{selected.bankAccount || "—"}</strong></span></div><div className={styles.opFormGrid}><label className={styles.opField}><span>Statement amount</span><input type="number" min="0" required value={form.statementAmount} onChange={(e) => setForm({ ...form, statementAmount: e.target.value })} /></label><label className={styles.opField}><span>Statement reference</span><input required value={form.statementReference} onChange={(e) => setForm({ ...form, statementReference: e.target.value })} /></label><label className={styles.opField}><span>Statement date</span><input type="date" required value={form.statementDate} onChange={(e) => setForm({ ...form, statementDate: e.target.value })} /></label><label className={styles.opField}><span>Statement bank account</span><input required value={form.statementBankAccount} onChange={(e) => setForm({ ...form, statementBankAccount: e.target.value })} /></label></div><label className={styles.opField}><span>Company Admin comparison file</span><select value={form.packetId} onChange={(e) => setForm({ ...form, packetId: e.target.value })}><option value="">Select document or upload statement</option>{safeArray(selected.packets).map((row: any) => <option key={row.id} value={row.id}>{date(row.createdAt)} — {row.adminMessage || "Comparison file"}</option>)}</select></label><label className={styles.opUpload}><input type="file" accept=".pdf,.csv,.xls,.xlsx,.jpg,.jpeg,.png,.webp" onChange={(e) => void file(e)} /><span>{form.bankStatementUrl ? "Statement uploaded" : "Upload bank statement"}</span></label><label className={styles.opField}><span>Review reason or note</span><textarea value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} /></label><button className={styles.opPrimaryButton} disabled={busy}>{busy ? "Comparing…" : "Compare and verify deposit"}</button></> : <Empty text="Select a deposit." />}</form></div>;
  }
  ...
  function Reports({ filter, mode, notify }: { filter: Filter; mode: "FINANCIAL" | "PERFORMANCE"; notify: (message: string) => void }) {
    ...
    return <div className={styles.opStack}><div className={styles.opReportToolbar}><button onClick={() => void preview()}>{loading ? "Loading…" : "Refresh preview"}</button><button onClick={() => window.print()}>Print preview</button><button onClick={() => download("pdf")}>Export PDF</button><button onClick={() => download("csv")}>Export CSV</button><button onClick={() => download("xlsx")}>Export Excel</button></div>{report ? <><div className={styles.opMetrics}><Metric label="Total income" value={money(report.summary?.totalIncome)} note={report.summary?.period} /><Metric label="Approved expenses" value={money(report.summary?.totalExpenses)} /><Metric label="Net income" value={money(report.summary?.netIncome)} /><Metric label="Verified deposits" value={money(report.summary?.totalDeposits)} /></div>
      <Table headers={mode === "FINANCIAL" ? ["STAFF user", "Income", "Approved expenses", "Net contribution", "Verified deposits", "Float", "Cash", "Transactions"] : ["STAFF user", "Present", "Absent", "Attendance rate", "Performance score", "Transactions", "Income", "Deposits"]}>{rows.map((row: any) => mode === "FINANCIAL" ? <tr key={row.userId}><td><Person row={row} /></td><td>{money(row.income)}</td><td>{money(row.approvedExpenses)}</td><td><strong>{money(Number(row.income || 0) - Number(row.approvedExpenses || 0))}</strong></td><td>{money(row.verifiedDeposits)}</td><td>{money(row.floatIssued)}</td><td>{money(row.cashIssued)}</td><td>{row.transactions}</td></tr> : <tr key={row.userId}><td><Person row={row} /></td><td>{row.attendancePresent}</td><td>{row.attendanceAbsent}</td><td>{row.attendanceRate}%</td><td><strong>{row.performanceScore}</strong></td><td>{row.transactions}</td><td>{money(row.income)}</td><td>{money(row.verifiedDeposits)}</td></tr>)}{!rows.length && <tr><td colSpan={8}><Empty text="No report data in this period." /></td></tr>}</Table></> : <Empty text="Generate the report preview." />}</div>;
  }
  ...
  function CardTitle({ title, text: description }: { title: string; text: string }) {
    return <header className={styles.opCardTitle}><span className={styles.cardHeaderIcon}><MaterialIcon text={title} /></span><div><h3>{title}</h3><p>{description}</p></div></header>;
  }

  function Detail({ label: title, value }: { label: string; value: unknown }) {
    return <article><span><MaterialIcon text={title} />{title}</span><strong>{text(value) || "—"}</strong></article>;
  }
