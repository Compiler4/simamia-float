...
async function requestJson<T = any>(
  ...
  // IMPORTANT:
  // Never set Content-Type manually for FormData. The browser must add the
  // multipart boundary itself, for example:
  ...
  try {
    ...
    throw new Error(
      `[${options?.method || "GET"} ${url}] The server returned invalid JSON (${response.status}).`,
    );
  }
...
function DocumentPreviewModal({ document, onClose }: { document: any; onClose: () => void }) {
  ...
  const url = reviewDocumentUrl(document?.publicUrl || document?.url || document?.storagePath);
